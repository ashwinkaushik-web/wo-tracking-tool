-- po_tracker.sql — PO Details source (UK/EU: Northampton + Wroclaw)
-- Base: ANALYTICS_DB.REPORTING.REPORT__BRAND_MANAGEMENT_V7__PURCHASE_ORDERS
--       (transient base table, rebuilt daily; grain = 1 row per PO x line item)
-- Scope: warehouses Northampton/Wroclaw, POs placed on/after 2025-07-01.
-- Fill rates are Tableau calcs (reproduced here, UNCAPPED — >100% = real over-receipts):
...
-- Label gotcha: report "Original Ordered" = ORDERED_UNITS; report "Ordered"/"Current" = CURRENT_UNITS.
SELECT
    ...
    , rpt.ORDERED_UNITS                      AS original_ordered_units   -- report "Original Ordered Units"
    , rpt.CURRENT_UNITS                      AS ordered_units            -- report "Ordered Units"
    , rpt.CURRENT_UNITS                      AS current_units            -- report "Current Units"
